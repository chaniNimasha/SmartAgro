...
export default function HistoryDaily({ navigation }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (token) {
                ...
                fetch('https://8e8b-112-134-155-12.ngrok-free.app/sensors/history?duration=daily', requestOptions)
                    ...
                    .then(data => {
                        ...
                        if (data.data.length > 0) {
                            ...
                            setPageTitle(`Sensor History (Daily) (Latest Update On ${latestDatetime})`);
                          }
                    })
                    .catch(error => {
    ...
    return (
        <ScrollView>
            <SafeAreaView style={styles.body}>
                ...
                {renderGraph(sensorData?.dtemp, 'Temperature Analysis (°C)')}
                {renderGraph(sensorData?.dhum, 'Humidity Analysis (%)')}
                {renderGraph(sensorData?.dsTemp, 'Soil Temperature Analysis (°C)')}
                {renderGraph(sensorData?.dsMois, 'Soil Moisture Analysis (%)')}
                {renderGraph(sensorData?.dsec, 'Soil EC Analysis (mS/cm)')}
                {renderGraph(sensorData?.dph, 'Soil PH Analysis')}
                {renderGraph(sensorData?.dn, 'Soil Nitrogen Analysis (ppm)')}
                {renderGraph(sensorData?.dp, 'Soil Phosphorus Analysis (ppm)')}
                {renderGraph(sensorData?.dk, 'Soil Potassium Analysis (ppm)')}
                {renderGraph(sensorData?.dr, 'rain Fall Analysis (mm)')}
            </SafeAreaView>
        </ScrollView>
    );
}
